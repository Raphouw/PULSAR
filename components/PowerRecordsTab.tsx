...
import { Zap, ChevronDown, Calendar, Medal, Crown, ArrowUpRight } from 'lucide-react';
...
export default function PowerRecordsTab({ rawRecords, userWeight = 68 }: { rawRecords: any[], userWeight?: number }) {
    ...
    const powerKeys = useMemo(() => {
        ...
        safeRecords.forEach(r => {
            ...
            const match = key.match(/^(?:power_|p)(\d+)(s|m|h)$/i);
            if (match) {
    ...
    if (powerKeys.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-gray-500 opacity-50">
                ...
                <p>Aucune donnée de puissance trouvée.</p>
            </div>
        );
    }
                ...
                {powerKeys.map(k => {
                    ...
                    return (
                        <div key={k.id} className={`bg-[#0a0a0c] border transition-all duration-300 rounded-2xl overflow-hidden shadow-lg ${isExpanded ? 'border-[#eab308]/30' : 'border-white/5 hover:border-white/10'}`}>
                            ...
                            {/* ⚡ SCROLLBAR INVISIBLE STYLISÉE ICI AUSSI */}
                            {isExpanded && (
