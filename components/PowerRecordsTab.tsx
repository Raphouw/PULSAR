    ...
    const powerKeys = useMemo(() => {
        ...
        safeRecords.forEach(r => {
            ...
            if (match) {
                ...
                let customLabel = seconds === 1 ? 'P-MAX (1S)' : `P-${val}${labelUnit.toUpperCase()}`;
                
                if (seconds === 5) customLabel = 'SPRINT (5S)';
    ...
    return (
        <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto w-full">
            ...
            <div className="flex bg-[#141419] p-1.5 rounded-xl border border-white/5 mb-2 overflow-x-auto hide-scrollbar">
                <button onClick={() => { setPowerMode('power_elapsed'); setExpandedKey(null); }} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${powerMode === 'power_elapsed' ? 'bg-[#eab308] text-black shadow-md' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}><Clock size={16} /> Elapsed (0s compris)</button>
                <button onClick={() => { setPowerMode('power_moving'); setExpandedKey(null); }} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${powerMode === 'power_moving' ? 'bg-[#eab308] text-black shadow-md' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}><Activity size={16} /> Moving (0s exclus)</button>
                <button onClick={() => { setPowerMode('power_np'); setExpandedKey(null); }} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${powerMode === 'power_np' ? 'bg-[#eab308] text-black shadow-md' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}><Flame size={16} /> Normalized Power (NP)</button>
            </div>
                    ...
                    {powerKeys.map(k => {
                        ...
                        return (
                            <div key={k.id} className={`bg-[#0a0a0c] border transition-all duration-300 rounded-2xl overflow-hidden shadow-lg ${isExpanded ? 'border-[#eab308]/30' : 'border-white/5 hover:border-white/10'}`}>
                                <button onClick={() => setExpandedKey(isExpanded ? null : k.id)} className="w-full p-4 flex items-center justify-between bg-[#141419] hover:bg-white/[0.02] transition-colors">
                                    <div className="flex items-center gap-4">
                                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${isExpanded ? 'bg-[#eab308]/20 text-[#eab308]' : 'bg-white/5 text-gray-500'}`}><Zap size={20} fill={isExpanded ? "currentColor" : "none"} /></div>
                                        <div className="text-left flex flex-col"><h3 className="font-black text-lg text-white uppercase tracking-wider">{k.label}</h3><span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{top10.length} efforts enregistrés</span></div>
                                    </div>
                                    <div className="flex items-center gap-6">
                                        <div className="hidden sm:flex flex-col items-end"><div className="text-xl font-black text-white flex items-baseline gap-1">{bestW} <span className="text-xs text-gray-500">W</span></div><div className="text-[10px] text-[#eab308] font-bold font-mono bg-[#eab308]/10 px-2 py-0.5 rounded">{bestWkg} W/kg</div></div>
                                    ...
                                    <div className="p-2 sm:p-4 border-t border-white/5 bg-black/40 max-h-[400px] overflow-y-auto [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-white/20">
                                        <div className="flex flex-col gap-1.5">
                                            {top10.map((record, index) => {
                                                ...
                                                return (
                                                    <button key={`${k.id}-${index}`} onClick={() => router.push(`/activities/${record.activity_id}`)} className="w-full flex items-center justify-between p-3 sm:p-4 rounded-xl hover:bg-white/[0.04] border border-transparent hover:border-white/5 transition-all group">
                                                        <div className="flex items-center gap-3 sm:gap-4">
                                                            <RankIcon rank={index + 1} />
                                                            <div className="flex flex-col items-start"><span className="text-sm font-bold text-white text-left line-clamp-1 group-hover:text-[#eab308] transition-colors max-w-[150px] sm:max-w-[300px]">{activityName}</span><div className="flex items-center gap-2 mt-0.5 text-[10px] text-gray-500 font-mono uppercase"><Calendar size={10} /> {date}</div></div>
                                                        </div>
                                                        <div className="flex items-center gap-4">
                                                            <div className="flex flex-col items-end"><span className="text-base font-black text-white group-hover:text-[#eab308] transition-colors">{w}W</span><span className="text-[10px] text-gray-500 font-mono">{wkg} w/kg</span></div>
