...
export default function PowerRecordsTab({ rawRecords, userWeight = 68 }: { rawRecords: any[], userWeight?: number }) {
    ...
    const powerKeys = useMemo(() => {
        ...
        safeRecords.forEach(r => {
            ...
            // ⚡ AJOUT : Gestion manuelle de la moyenne max (qui ne passe pas dans la regex des durées)
            if (key === 'pmoymax') {
                map.set(key, { id: key, label: 'P-MOY MAX', seconds: 999999 }); // 999999 pour l'afficher en dernier
                return;
            }
            ...
            if (match) {
                ...
                map.set(key, { id: key, label: `${val} ${labelUnit}`, seconds });
            }
        });
