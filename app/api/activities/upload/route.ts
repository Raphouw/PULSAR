...
export async function POST(req: Request) {
  try {
    ...
    // ⚡ FORCE TYPE: On cast la réponse car TypeScript est perdu avec "never"
    ...
    // On utilise un type intermédiaire pour éviter l'erreur
    ...
    const { data: duplicatesData } = await supabaseAdmin
      .from("activities")
    ...
    // ⚡ FORCE TYPE: On cast duplicatesData
    const duplicates = duplicatesData as any[];
    ...
    // ⚡ FORCE TYPE: On cast l'objet d'insertion en 'any' pour contourner le type 'never'
    const insertPayload = {
      user_id: userId,
    ...
    const { data: insertedData, error } = await supabaseAdmin
      .from("activities")
      .insert(insertPayload as any) // ⚡ Le 'as any' sauve l'insertion
      .select('id')
      .single();

    if (error) throw error;

    // ⚡ FORCE TYPE: On récupère l'ID inséré
    const activityData = insertedData as any;

    // --- TRIGGER SCAN SEGMENTS ---
    ...
    if (countWatts > 0) {
      ...
      if (recordsToInsert.length > 0) {
        // ⚡ FORCE TYPE: On cast aussi pour les records
        await supabaseAdmin.from("records").insert(recordsToInsert as any);
      }
    }
